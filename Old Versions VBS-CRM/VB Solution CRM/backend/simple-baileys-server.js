...
function previewLabel(tipo, conteudo, duration_ms) {
...
async function createWhatsAppConnection(connectionId, userId = '00000000-0000-0000-0000-000000000000') {
  try {
    console.log(`🔗 Criando conexão WhatsApp para: ${connectionId} (usuário: ${userId})`);
    
    // Verificar se já existe uma conexão ativa
    if (activeConnections.has(connectionId)) {
      console.log(`⚠️ Conexão ${connectionId} já existe, removendo...`);
      const existingConnection = activeConnections.get(connectionId);
      if (existingConnection.sock) {
        existingConnection.sock.logout();
      }
    ...
    console.log('🔐 Configurando estado de autenticação...');
    ...
    sock.ev.on('connection.update', async (update) => {
      ...
      if (qr) {
        ...
        // Atualizar conexão na memória
        const connection = activeConnections.get(connectionId);
        if (connection) {
          connection.qrCode = qr;
          connection.connectionState = 'connecting';
          console.log(`✅ QR code salvo na memória para ${connectionId}`);
        }
        ...
        try {
          ...
          console.log(`✅ QR code salvo no Supabase para ${connectionId}`);
        } catch (error) {
          console.error(`❌ Erro ao salvar QR code no Supabase:`, error);
        }
        ...
        console.log(`📡 QR code emitido via Socket.IO para ${connectionId}`);
      }
    ...
    sock.ev.on('messages.upsert', async (m) => {
      try {
        ...
        for (const message of messages) {
          ...
          const messageData = mapToDbRow(message, null, connectionId, 'default-owner');
          ...
          try {
            if (isMediaTipo(messageData.tipo)) {
              const keyPrefix = [
              ...
              messageData.media_type = messageData.media_type || uploaded.mime;
            }
          } catch (err) {
          ...
          const preview = previewLabel(messageData.tipo, messageData.conteudo, messageData.duration_ms);
    ...
    // Armazenar conexão
    console.log(`💾 Armazenando conexão ${connectionId} na memória...`);
    ...
    console.log(`✅ Conexão ${connectionId} armazenada na memória`);
    
    // Salvar sessão inicial no Supabase
    console.log(`💾 Salvando sessão inicial no Supabase...`);
    try {
      await saveSessionToSupabase(connectionId, {
        name: connectionData.name,
        status: 'disconnected'
      }, userId);
      console.log(`✅ Sessão inicial salva no Supabase para ${connectionId}`);
    } catch (error) {
      console.error(`❌ Erro ao salvar sessão inicial no Supabase:`, error);
    }
    
    console.log(`🎉 Conexão ${connectionId} criada com sucesso!`);
    return sock;
  } catch (error) {
...
app.get('/api/baileys-simple/connections/:connectionId/qr', async (req, res) => {
  try {
    ...
    console.log(`🔍 Buscando QR code para conexão: ${connectionId}`);
    
    const connection = activeConnections.get(connectionId);
    
    if (!connection) {
      console.log(`❌ Conexão ${connectionId} não encontrada na memória`);
    ...
    console.log(`📊 Status da conexão: ${connection.connectionState}, Conectado: ${connection.isConnected}`);
    
    // Se já estiver conectado, não retornar QR code
    ...
    if (!qrCode) {
      console.log(`🔍 Buscando QR code no Supabase para ${connectionId}`);
      ...
      if (error) {
        console.error(`❌ Erro ao buscar QR code no Supabase:`, error);
      } else if (session) {
        qrCode = session.qr_code;
        console.log(`✅ QR code encontrado no Supabase: ${qrCode ? 'Sim' : 'Não'}`);
      }
    } else {
      console.log(`✅ QR code encontrado na memória`);
    }
    
    // Se ainda não há QR code, aguardar um pouco e tentar novamente
    if (!qrCode) {
    ...
    console.log(`✅ Retornando QR code para ${connectionId}`);
    res.json({
      success: true,
      data: {
        qrCode: qrCode,
...
app.post('/api/baileys-simple/connections/:connectionId/send-message', async (req, res) => {
  try {
    ...
    const messageData = {
      owner_id: process.env.WA_OWNER_ID || 'default-owner',
      chat_id: chatId,
      conteudo: message,
