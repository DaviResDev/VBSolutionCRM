...
export const ConnectionsProvider: React.FC<ConnectionsProviderProps> = ({ children }) => {
  ...
  const generateQRCode = async (connectionId: string) => {
    try {
      ...
      if (connection.type === 'whatsapp_baileys') {
        // Para Baileys, criar conexão e obter QR Code
        ...
        if (!createResult.success) {
          throw new Error(createResult.error || 'Erro ao criar conexão Baileys');
        }
        ...
        // Obter QR Code
        const qrResponse = await fetch(`/api/baileys-simple/connections/${connectionId}/qr`);
        ...
        if (qrResult.success && qrResult.data.qrCode) {
          setConnections(prev => 
            prev.map(conn => 
              conn.id === connectionId 
                ? { ...conn, qrCode: qrResult.data.qrCode }
                : conn
            )
          );
          ...
          return { success: true, qrCode: qrResult.data.qrCode };
        } else {
