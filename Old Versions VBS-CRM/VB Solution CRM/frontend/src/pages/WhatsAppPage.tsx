...
const ConversationsList = ({ onConversationSelect, selectedConversationId }) => {
  const { socket, activeConnection } = useConnections();
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter(c =>
      ...
      (c.lastPreview || '').toLowerCase().includes(s)
    );
  }, [items, q]);
  ...
  if (filtered.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-64 text-gray-500 p-4">
        ...
        <button onClick={reload} className="px-3 py-1 text-xs bg-green-500 text-white rounded hover:bg-green-600">
          Recarregar
        </button>
      </div>
    );
    ...
    <div className="flex flex-col h-full">
      <div className="p-3 border-b border-gray-200 bg-gray-50">
        <div className="flex items-center justify-between">
          ...
          <button onClick={reload} className="text-xs text-green-600 hover:text-green-700 font-medium">Atualizar</button>
        </div>
