...
export default function BaileysQRModal({ 
  ...
  const fetchQRCode = async () => {
    if (!connectionId) {
      setError('ID de conexão não fornecido');
      setIsLoading(false);
      return;
    }
    ...
    try {
      ...
      const response = await fetch(`http://localhost:3000/api/baileys-simple/connections/${connectionId}/qr`);
      ...
      if (data.success) {
        if (data.data.qrCode) {
          ...
          setError(data.data.message || 'Aguardando QR Code...');
        }
      } else {
  ...
  const checkConnectionStatus = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:3000/api/baileys-simple/connections/${connectionId}/info`);
